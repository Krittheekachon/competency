...
</script><template><Head title="โปรไฟล์" /><div class="shell" :class="{ 'sidebar-hidden': !showSidebar }"><aside v-if="showSidebar" class="sidebar"><div class="sb-logo"><div class="sb-mark">คณะวิศวกรรมศาสตร์</div><div class="sb-name">
Competency &<br />
IDP Management
 </div></div><button class="sb-user on" type="button"><div class="av" :style="{ background: roleData.col }">
{{ fullName[0] || roleData.av }}
 </div><div style="overflow: hidden; min-width: 0"><div class="u-name">{{ fullName }}</div><div class="u-role">{{ form.position || roleData.pos }}</div></div></button><div class="sb-nav"><div v-for="(section, sectionIndex) in navSections" :key="sectionIndex"><div class="nav-sec">{{ section.sec }}</div><button
...
{{ item.lb }}
 </button></div></div></aside><main class="main"><div class="topbar"><button class="menu-btn" type="button" @click="showSidebar = !showSidebar">☰</button><div class="tb-title">โปรไฟล์</div><span class="tb-badge">รอบประเมิน 2568</span><button class="btn btn-s btn-sm" style="margin-left: 8px" type="button" @click="logout">
ออกจากระบบ
 </button></div><form class="content profile-page" @submit.prevent="saveProfile"><section class="profile-hero card"><div class="profile-identity"><div class="profile-photo-block"><div class="profile-avatar"><img
v-if="form.profile_photo"
:src="form.profile_photo"
:alt="fullName"
/><span v-else>{{ fullName[0] || roleData.av }}</span></div><div v-if="isEditing" class="photo-actions"><input
ref="photoInput"
type="file"
accept="image/*"
class="photo-input"
...
{{ form.profile_photo ? 'เปลี่ยนรูป' : 'เพิ่มรูป' }}
 </button></div></div><div class="profile-heading"><div class="sec-t">โปรไฟล์บุคลากร</div><h1>{{ fullName }}</h1><p>{{ roleLine }}</p><div class="profile-tags"><span>ID {{ form.sso || '—' }}</span><span>{{ form.workline || 'ยังไม่ได้ระบุสายงาน' }}</span></div></div></div><div class="profile-actions"><template v-if="isEditing"><button class="btn btn-s" type="button" @click="cancelEdit">ยกเลิก</button><button class="profile-primary-btn" type="submit" :disabled="form.processing">
{{ form.processing ? 'กำลังบันทึก...' : 'บันทึกโปรไฟล์' }}
 </button></template><button v-else class="profile-primary-btn" type="button" @click="isEditing = true">
แก้ไขโปรไฟล์
 </button></div></section><section class="profile-section card"><div class="fw8 fs14">ข้อมูลบุคลากร</div><div v-if="status" class="profile-status">{{ status }}</div><div class="profile-grid" :class="{ editing: isEditing }"><div v-for="field in fieldGroups" :key="field.key" class="profile-field"><label class="lbl">
{{ field.label }} <span v-if="field.required" class="required">*</span></label><template v-if="isEditing"><select
...
{{ form.errors[field.key] }}
 </div></template><div v-else class="profile-value">
{{ displayValue(field) || '—' }}
 </div></div></div></section></form></main></div></template><style scoped>
.nav-button {
width: 100%;
}
