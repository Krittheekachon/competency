<template><!-- ── Header ───────────────────────────────────────────────────────────── --><div class="flex ic jb mb20"><div><div class="sec-t">รายละเอียด IDP </div><div class="sec-s">ภาพรวม · Timeline · ประวัติย้อนหลัง</div><div
...
><span style="font-size: 12px"></span><span style="font-size: 12px; font-weight: 700; color: #fff">รอบประเมิน 2568</span><span style="font-size: 11px; color: rgba(255,255,255,.6)">รอบปัจจุบัน</span></div></div><button class="btn btn-s"> Export PDF</button></div><!-- ── Summary stats ─────────────────────────────────────────────────────── --><div class="g4 mb20"><div class="sc" style="border-top: 3px solid var(--navy)"><div class="sl">กิจกรรมทั้งหมด</div><div class="sv" style="color: var(--navy)">{{ cntTotal }}</div><div class="ss muted">รอบปัจจุบัน</div></div><div class="sc" style="border-top: 3px solid var(--green)"><div class="sl">เสร็จสิ้น</div><div class="sv gcc">{{ cntDone }}</div><div class="ss muted">กิจกรรม</div></div><div class="sc" style="border-top: 3px solid var(--blue)"><div class="sl">กำลังดำเนินการ</div><div class="sv bc">{{ cntInprog }}</div><div class="ss muted">กิจกรรม</div></div><div class="sc" style="border-top: 3px solid var(--red)"><div class="sl">ไม่ผ่าน</div><div class="sv rc">{{ cntFailed }}</div><div class="ss muted">กิจกรรม</div></div></div><!-- ── Timeline card ─────────────────────────────────────────────────────── --><div class="card mb20"><div class="ch"><div><div class="ct"> Timeline กิจกรรม IDP</div><div class="cs">กดที่กิจกรรมเพื่อดูรายละเอียดและหลักฐาน</div></div></div><div class="cb" style="padding: 0"><div
...
>
{{ act.ic }}
 </div><div style="flex: 1; min-width: 0"><div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap"><span class="fw7 fs13">{{ act.t }}</span><span :class="gapTagClass(act.gapT)" style="font-size: 9px">{{ act.gapT }}</span></div><div style="display: flex; align-items: center; gap: 8px; margin-top: 4px; flex-wrap: wrap"><span style="font-size: 11px; color: var(--text3)">{{ act.gapCd }} · {{ act.gapN }}</span></div><div style="display: flex; align-items: center; gap: 6px; margin-top: 4px; flex-wrap: wrap"><span
...
>
{{ act.m }}
 </span><span style="font-size: 11px; color: var(--text3)">ครบ {{ act.due }}</span></div></div><div style="text-align: center; flex-shrink: 0"><div style="font-size: 18px; font-weight: 800" :style="{ color: getResultMeta(act.result).color }">
{{ act.logs.length }}
 </div><div style="font-size: 10px; color: var(--text3)">บันทึก</div></div><span :class="`b ${getResultMeta(act.result).badge}`" style="flex-shrink: 0">
{{ getResultMeta(act.result).label }}
 </span><span style="font-size: 12px; color: var(--text3); flex-shrink: 0">
{{ expandedId === act.id ? '▾' : '▸' }}
 </span></div><!-- Failed inline banner --><div
...
><span style="font-size: 16px; flex-shrink: 0"></span><div><div class="fw7 fs12" style="color: var(--red)">
ไม่ผ่าน · {{ act.rejectedBy }} · {{ act.rejectedDate }}
 </div><div style="font-size: 12px; color: #991B1B; margin-top: 3px">
"{{ act.rejectComment }}"
 </div></div></div><!-- Expanded log panel --><div
v-if="expandedId === act.id"
style="background: var(--bg); border-top: 1px solid var(--border)"
><div style="padding: 14px 18px 6px"><div
style="font-size: 11px; font-weight: 700; color: var(--text3); text-transform: uppercase; letter-spacing: .08em; margin-bottom: 10px"
>
ประวัติการดำเนินงาน
 </div><div style="position: relative; padding-left: 28px"><div
...
><div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 8px"><div><div
...
by {{ log.by }}
 </div></div><div style="font-size: 11px; color: var(--text3); font-weight: 600; white-space: nowrap; flex-shrink: 0">
{{ log.d }}
 </div></div><div
v-if="log.evidence"
style="margin-top: 8px; padding: 6px 10px; background: var(--blue-lt); border-radius: 6px; display: flex; align-items: center; gap: 6px"
><span style="font-size: 14px"></span><span style="font-size: 11px; color: var(--blue); font-weight: 600">{{ log.evidence }}</span><span style="font-size: 10px; color: var(--text3); margin-left: auto">คลิกเพื่อดู →</span></div></div></div></div></div></div></div></div></div><!-- ── History card ───────────────────────────────────────────────────────── --><div class="card"><div class="ch" style="flex-direction: column; align-items: flex-start; gap: 12px"><div class="ct"> ประวัติ IDP ย้อนหลัง</div><div style="display: flex; gap: 6px; flex-wrap: wrap"><button
...
{{ round.n }}{{ round.isCurrent ? ' (ปัจจุบัน)' : '' }}
 </button></div></div><div class="cb" style="padding-top: 0"><!-- Current round --><div v-if="activeRoundData.isCurrent" style="padding: 16px 0"><div
v-for="gap in currentGaps"
:key="gap.cd"
style="padding: 14px 0; border-bottom: 1px solid var(--border)"
><div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap"><span :class="gapTagClass(gap.t)">{{ gap.t }}</span><span class="fw7 fs13">{{ gap.n }}</span><span class="muted fs12">{{ gap.cd }} · Gap {{ gap.gapValue }}</span><span :class="`b ${getStatusMeta(gap.status).cls}`" style="margin-left: auto">
{{ getStatusMeta(gap.status).label }}
 </span></div><div style="margin-top: 10px; display: flex; align-items: center; gap: 10px"><div class="pw" style="flex: 1; height: 6px"><div
class="pb"
:style="{
width: `${gap.pct}%`,
background: gap.pct === 100 ? 'var(--green)' : 'var(--blue)',
}"
/></div><span style="font-size: 11px; color: var(--text3)">
...
><span style="font-size: 16px">{{ act.ic }}</span><div style="flex: 1"><div class="fw6 fs12">{{ act.t }}</div><div class="muted fs11">{{ act.m }} · ครบ {{ act.due }}</div></div><span :class="`b ${getResultMeta(act.result).badge}`">
{{ getResultMeta(act.result).label }}
 </span><span class="muted fs11">{{ act.logs.length }} บันทึก</span></div></div></div></div><!-- Past rounds --><div v-else style="padding: 16px 0"><div
...
><span></span><span style="font-size: 12px; font-weight: 700; color: var(--green)">
{{ activeRoundData.n }} — เสร็จสิ้นแล้ว
 </span></div><div
v-for="(gap, idx) in activeRoundData.gaps || []"
:key="`${gap.cd}-${idx}`"
style="padding: 14px 0; border-bottom: 1px solid var(--border)"
><div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-bottom: 8px"><span :class="gapTagClass(gap.t)">{{ gap.t }}</span><span class="fw7 fs13">{{ gap.n }}</span><span class="muted fs12">{{ gap.cd }} · Gap {{ gap.gap }}</span><span class="b bg" style="margin-left: auto">เสร็จสิ้น</span></div><div
v-for="(act, actIdx) in gap.acts"
:key="actIdx"
style="margin-bottom: 8px; padding: 8px 12px; background: var(--bg); border-radius: var(--r); border-left: 3px solid var(--green)"
><div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px"><span style="font-size: 15px">{{ act.ic }}</span><span class="fw6 fs12">{{ act.t }}</span><span class="muted fs11">{{ act.m }}</span><span class="b bg" style="margin-left: auto; font-size: 9px">ผ่านแล้ว ✓</span></div><div
v-for="(log, logIdx) in act.logs"
:key="logIdx"
...
><span style="font-size: 12px"></span><span style="font-size: 11px; color: var(--blue); font-weight: 600">{{ log.evidence }}</span><span style="font-size: 10px; color: var(--text3)">↗</span></div></div><span style="font-size: 10px; color: var(--text3); flex-shrink: 0; white-space: nowrap">
by {{ log.by }}
 </span></div></div></div></div></div></div></template><script setup lang="ts">
