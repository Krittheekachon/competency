<template><!-- ── Header ───────────────────────────────────────────────────────────── --><div class="mb20"><div class="sec-t">อัปเดตความก้าวหน้า</div><div class="sec-s">บันทึกผลการพัฒนา แนบหลักฐาน และอัปเดตสถานะกิจกรรม IDP</div></div><div v-if="totalActivities === 0" class="card empty-progress"><div class="empty-icon"></div><div class="empty-title">ยังไม่มีกิจกรรมให้ติดตามความก้าวหน้า</div><div class="empty-desc">
เมื่อมีการเพิ่มกิจกรรมในแผน IDP แล้ว รายการสำหรับบันทึกความก้าวหน้าจะแสดงที่หน้านี้
 </div></div><!-- ── Gap sections ──────────────────────────────────────────────────────── --><div v-for="{ g, acts } in list" v-else :key="g.cd" style="margin-bottom: 24px"><!-- Gap header --><div
...
><span v-if="hasFailed(acts)" style="font-size: 16px; flex-shrink: 0">!</span><span :class="gapTagClass(g.t)">{{ g.t }}</span><span class="fw7 fs13" :style="{ color: hasFailed(acts) ? 'var(--red)' : 'inherit' }">
{{ g.n }}
 </span><div style="display: flex; align-items: center; gap: 6px; margin-left: auto"><span v-if="hasFailed(acts)" class="b br">มีกิจกรรมไม่ผ่าน</span><span :class="`b ${g.pri === 'high' ? 'br' : 'by'}`">
{{ g.pri === 'high' ? 'เร่งด่วน' : 'ต้องพัฒนา' }}
 </span></div></div><!-- Activity cards --><div
v-for="(act, aIdx) in acts"
:key="aIdx"
class="card mb10"
><!-- Card header --><div class="ch"><div
...
>
{{ act.ic }}
 </div><div style="flex: 1; margin-left: 10px"><div class="fw7 fs13">{{ act.t }}</div><div class="muted fs12 mt4">{{ act.m }} · ครบ {{ act.due }}</div></div><span :class="`b ${act.stC}`">{{ act.st }}</span></div><!-- Failed banner --><div
...
><div style="font-size: 20px; flex-shrink: 0">!</div><div style="flex: 1"><div class="fw7 fs13" style="color: var(--red); margin-bottom: 2px">กิจกรรมนี้ไม่ผ่าน</div><div class="fs12" style="color: #991B1B; margin-bottom: 6px"><span class="fw6">{{ act.rejectedBy }}</span> · {{ act.rejectedDate }}
 </div><div
class="fs12"
style="background: #fff; border: 1px solid #FECACA; border-radius: 6px; padding: 8px 10px; color: var(--text2); margin-bottom: 10px"
>
"{{ act.rejectComment }}"
 </div></div></div><!-- Card body --><div class="cb" style="padding-top: 10px"><!-- Progress form (hidden when failed) --><div
v-if="act.result !== 'failed'"
style="background: var(--bg); border: 1px solid var(--border); border-radius: var(--r); padding: 12px; margin-bottom: 12px"
><div class="fw7 fs12 mb8">บันทึกความก้าวหน้าใหม่</div><!-- Note textarea --><div class="fg mb8"><label class="lbl" style="font-weight: 500; font-size: 11px">บันทึก</label><textarea
class="ta"
...
/></div><!-- Evidence section --><div class="g2 mb8"><!-- File upload --><div><div class="lbl mb8" style="font-size: 11px">
แนบหลักฐาน <span class="lbl-opt">(ถ้ามี)</span></div><label
class="upload-area"
...
/><div style="font-size: 18px; margin-bottom: 4px"></div><div class="fw6 fs12">
{{ getForm(g.cd, aIdx).fileName || 'คลิกเพื่อแนบไฟล์' }}
 </div><div class="muted fs11">PDF, Word, รูปภาพ</div></label></div><!-- URL + description --><div><div class="fg"><label class="lbl" style="font-weight: 500; font-size: 11px">URL หลักฐาน</label><input
class="inp"
style="font-size: 12px"
placeholder="https://..."
:value="getForm(g.cd, aIdx).evidenceUrl"
@input="e => setForm(g.cd, aIdx, { evidenceUrl: (e.target as HTMLInputElement).value })"
/></div><div class="fg mb0"><label class="lbl" style="font-weight: 500; font-size: 11px">คำอธิบาย</label><textarea
class="ta"
...
/></div></div></div><!-- Action buttons --><div style="display: flex; justify-content: flex-end; gap: 6px"><button class="btn btn-s btn-sm" type="button" @click="saveProgress(g.cd, aIdx, 'draft')">
บันทึกร่าง
 </button><button class="btn btn-t btn-sm" type="button" @click="saveProgress(g.cd, aIdx, 'saved')">
บันทึก
 </button></div></div><!-- Log history --><div class="fw7 fs12 mb8 muted">ประวัติการอัปเดต</div><div
...
{{ L.d }}
 </div><div style="flex: 1; font-size: 12px">{{ L.n }}</div><span class="muted fs11">by {{ L.by }}</span></div></div></div></div></template><script setup lang="ts">
