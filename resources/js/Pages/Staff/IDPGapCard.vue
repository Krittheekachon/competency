<template>
  <div class="idp-gap" style="margin-bottom: 20px; scroll-margin-top: 110px; overflow: hidden">
    <!-- ── Header ─────────────────────────────────────────────────────────── -->
    <div class="idp-gap-h" style="padding: 14px 18px">
      <div style="display: flex; align-items: center; gap: 10px; flex: 1; flex-wrap: wrap">
        <span :class="`b ${gap.pri === 'high' ? 'br' : 'by'}`">
          {{ gap.pri === 'high' ? 'เร่งด่วน' : 'ต้องพัฒนา' }}
        </span>
        <span :class="gapTagClass">{{ gap.t }}</span>
        <span class="fw8 fs14">{{ gap.n }}</span>
        <span class="muted fs12" style="margin-left: 4px">
      ...
      <div style="display: flex; align-items: center; gap: 8px">
        ...
        <span v-else :class="`b ${statusMeta.cls}`">{{ statusMeta.badge }}</span>
      </div>
    </div>
    ...
    >
      ...
      <div style="flex: 1">
        <div class="fw7 fs13" style="color: var(--red); margin-bottom: 3px">
          แผนนี้ไม่ผ่านการอนุมัติ
        </div>
        <div class="fs12" style="color: #991B1B; margin-bottom: 6px">
          <span class="fw6">{{ gap.rejectedBy }}</span> {{ gap.rejectedDate }}
        </div>
        <div
          class="fs12"
      ...
      <button class="btn btn-r btn-sm" style="flex-shrink: 0">แก้ไขแผน</button>
    </div>
    ...
    >
      <label class="lbl">
        เป้าหมายการพัฒนา <span style="color: var(--red)">*</span>
      </label>
      <textarea
        class="ta"
