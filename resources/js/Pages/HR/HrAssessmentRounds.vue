...
const resetForm = () => {
    ...
    form.reset();
    form.year = new Date().getFullYear() + 543;
        ...
        <div class="summary-grid">
            <section class="summary-card active-card">
                ...
                <template v-if="activeRound">
                    ...
                    <span class="active-year">ปี {{ activeRound.year }}</span>
                </template>
                <template v-else>
                    <strong class="active-name empty">ยังไม่มีรอบที่เปิดใช้งาน</strong>
                    <span class="active-year">กด “เปิดรอบใหม่” เพื่อเริ่มต้น</span>
                </template>
            </section>
                ...
                >
                    <header class="editor-heading">
                        <div>
                            ...
                            <p>ระบุชื่อ ปี และช่วงเวลาที่บุคลากรกับหัวหน้าดำเนินการได้</p>
                        </div>
                    ...
                    <div class="editor-grid">
                        <label class="field name-field">
                            ...
                            <input ref="nameInput" v-model="form.name" type="text" autocomplete="off" placeholder="เช่น รอบประเมินประจำปี 2569">
                            <em v-if="form.errors.name">{{ form.errors.name }}</em>
                        </label>
                        <label class="field">
                            <span>ปีการประเมิน *</span>
                        ...
                        <label v-if="!isEditing && rounds.length" class="field copy-field">
                            ...
                            <select v-model="form.copy_from_round_id">
                                ...
                                <option v-for="round in rounds" :key="round.id" :value="round.id">{{ round.name }} ({{ round.year }})</option>
                            </select>
