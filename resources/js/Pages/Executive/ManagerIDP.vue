...
return (
 <><div class="mb20"><div class="sec-t">ภาพรวม IDP คณะ </div><div class="sec-s">
...
><div style={{ flex: 1, minWidth: "190px" }}><div class="fw7 fs12 muted mb6">บุคลากรไม่ผ่านเกณฑ์ที่มี IDP แล้ว</div><div
style={{
display: "flex",
alignItems: "baseline",
gap: "8px",
...
>
{pctIDP}%
 </span></div><div class="fs12 muted mt4">
ยังไม่ได้ทำ IDP อีก{" "}
 <span class="fw7 rc">{idpStats.noIdp} คน</span></div></div><div style={{ flex: 2, minWidth: "280px" }}><div class="fs11 fw7 muted mb6">สัดส่วนตามสถานะ</div><div
style={{
height: "20px",
borderRadius: "6px",
overflow: "hidden",
...
>
{statLegend.map((item) => (
 <div key={item.label} class="flex ic g4"><span
style={{
width: "10px",
height: "10px",
borderRadius: "3px",
...
{item.label} <b>{item.value}</b></span></div>
))}
 </div></div></div></div></div>

{/* Stat cards */}
 <div class="g4 mb14"><div class="sc"><div class="sl">เสร็จสิ้น</div><div class="sv gcc">{idpStats.completed}</div><div class="ss muted">คน</div></div><div class="sc"><div class="sl">กำลังดำเนินการ</div><div class="sv bc">{idpStats.inProgress}</div><div class="ss muted">คน</div></div><div class="sc"><div class="sl">รออนุมัติ</div><div class="sv yc">{idpStats.submitted}</div><div class="ss muted">คน</div></div><div class="sc"><div class="sl">Draft / ยังไม่มี IDP</div><div class="sv rc">{idpStats.draft + idpStats.noIdp}</div><div class="ss muted">
...
{/* Two-column section */}
 <div class="g2 mb14">
{/* Progress by dept */}
 <div class="card"><div class="ch"><div class="ct">ความคืบหน้า IDP รายหน่วยงาน</div></div><div class="cb" style={{ padding: 0 }}>
...
><div class="flex ic g8 mb8"><span class="fw6 fs13" style={{ flex: 1 }}>
{group.d}
 </span><span class="fw7 fs12" style={{ color: barColor }}>
...
>
ดูรายละเอียด
 </button></div><div class="flex ic g8"><div class="pw" style={{ flex: 1, height: "7px", overflow: "hidden" }}><div
class="pb"
...
{/* No progress list */}
 <div class="card"><div class="ch"><div><div class="ct">บุคลากรที่ยังไม่มีความคืบหน้า IDP</div><div class="cs">{noProgress.length} คน</div></div><button
...
>
แจ้งเตือนทั้งหมด
 </button></div><div class="cb" style={{ padding: 0 }}>
{noProgress.map((item) => {
...
>
{item.n[0]}
 </div><div style={{ flex: 1 }}><div class="fw6 fs12">
{user ? `${user.t}${user.n}` : item.n}
 </div><div class="muted fs11">
{item.pos} · {item.d}
 </div></div><span
class={`b ${meta.badge}`}
style={{
...
><div class="mo-h"><div class="fw8 fs14">
...
>
{item.n[0]}
 </div><div style={{ flex: 1 }}><div class="fw6 fs13">
{user ? `${user.t}${user.n}` : item.n}
 </div><div class="muted fs11">
{item.pos} · เรื่อง: {item.topic}
 </div></div><span class={`b ${meta.badge}`}>{meta.label}</span></div>
);
})}
 </div></div></div>
)}
