...
{row.position} · {row.dept}
 </div></td><td><div class="fw6 fs12">{row.evaluator1 || "—"}</div></td><td><div class="fw6 fs12">{row.evaluator2}</div></td><td style={{ textAlign: "center" }}><span class="fw8" style={{ color: "var(--blue)" }}>
{row.score}
 </span><span class="muted fs10"> / 5</span></td><td><span class={`b ${approved ? "bg" : "by"}`}>
...
><div class="fw7 fs13">
{item.n}
 <div class="muted fs11">{item.note}</div></div><span class="b bgr" style={{ justifyContent: "center" }}>
...
>
ได้ {item.actual}
 </span><span class="muted fs11">ส่งเมื่อ {selected.submittedAt}</span></div>
))}
 <button
class={`btn ${
