        ...
        return (
          <div key={type} style={{ marginBottom: "24px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px", paddingBottom: "8px", borderBottom: "2px solid var(--border)" }}>
                            ...
                            <span style={{ marginLeft: "auto", fontSize: "11px", color: "var(--text3)" }}>{items.filter((_, i) => scores.value[itemCounter + i]).length}/{items.length} กรอกแล้ว</span>
                        </div>
                        {items.map((it, idx) => {
              const curIdx = itemCounter++;
              const score = scores.value[curIdx];
              return (
                <div key={it.cd} class="ac" style={{ marginBottom: "8px" }}>
                                    <div class="ah" onClick={() => setExpanded(expanded.value === curIdx ? null : curIdx)} style={{ background: '#fff' }}>
                                        <span class={config.tag} style={{ flexShrink: 0 }}>{config.tagLabel}</span>
                                        <span class="fw7 fs13" style={{ flex: 1, marginLeft: "2px" }}>{it.n}</span>
                                        <span style={{ fontSize: "12px", fontWeight: 700, color: score ? "var(--teal)" : "var(--red)" }}>
                                            {score ? score + " / 5 ✓" : "ยังไม่กรอก"}
                                        </span>
                                        <span style={{ marginLeft: "10px", color: "var(--text3)", fontSize: "12px" }}>{expanded.value === curIdx ? "▴" : "▾"}</span>
                                    </div>
                                    <div class={`ab ${expanded.value === curIdx ? "open" : ""}`}>
                                        <div style={{ marginBottom: "14px" }}>
                                            <div class="lbl mb6" style={{ fontSize: "11px" }}>พฤติกรรมบ่งชี้ (ใช้ประกอบการตัดสิน)</div>
                                            {score ?
                      <>
                      ...
                      <div class="muted fs12">เลือกคะแนนความสามารถเพื่อดูพฤติกรรมบ่งชี้ของระดับนั้น</div>
                                        ...
                                        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "8px", marginBottom: "14px" }}>
                                            {[1, 2, 3, 4, 5].map((k) =>
                      <div key={k} onClick={() => setScores({ ...scores.value, [curIdx]: k })} style={{ border: `2px solid ${score === k ? "var(--teal)" : "var(--border)"}`, borderRadius: "10px", padding: "14px 8px", cursor: "pointer", background: score === k ? "var(--teal-lt)" : "#fff", transition: ".15s", textAlign: "center" }}>
                                        ...
                                        <div class="g2">
                                            ...
                                            <div>
                                                <div class="fg">
                                                    <label class="lbl" style={{ fontWeight: 500, fontSize: '11px' }}>URL หลักฐาน</label>
                                                    <input class="inp" style={{ fontSize: '12px' }} placeholder="https://..." />
                                                </div>
                                                <div class="fg mb0">
                                                    <label class="lbl" style={{ fontWeight: 500, fontSize: '11px' }}>คำอธิบาย</label>
                                                    <textarea class="ta" style={{ minHeight: '48px', fontSize: '12px' }} placeholder="อธิบายสั้นๆ..." />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
export const EmployeeGap = defineComponent({ name: "EmployeeGap", props: Object as PropType<{setPage: (p: string) => void;}>, setup(__props) {const { setPage } = __props as any;
    ...
    const needIDP = gaps.filter((g) => g.sup < g.exp).sort((a, b) => a.sup - a.exp - (b.sup - b.exp));return () =>
    ...
    <>
            ...
            <div class="g2 mb14">
                <div class="sc" style={{ borderLeft: '4px solid var(--teal)' }}>
                ...
                <div class="sc" style={{ borderLeft: '4px solid var(--red)' }}>
            ...
            <div class="card mb14">
                ...
                return (
                  <tr key={i}>
                            ...
                            <td style={{ textAlign: 'center' }}><span class={g.t === 'CC' ? 'tag-cc' : g.t === 'MC' ? 'tag-mc' : g.t === 'FC1' ? 'tag-fc1' : g.t === 'FC2' ? 'tag-fc2' : 'tag-fc'}>{g.t}</span></td>
            ...
            <div class="card">
                <div class="ch">
                    ...
                    <div style={{ marginLeft: 'auto' }}>
                        <button class="btn btn-t btn-sm" onClick={() => setPage('emp-idp')}>สร้าง IDP →</button>
                    </div>
                </div>
                <div class="cb">
                    {needIDP.map((g, i) =>
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <span class="muted fs12" style={{ marginLeft: "8px" }}>{g.cd}</span>
                            </div>
                            <span class={`b ${g.t === 'CC' ? 'tag-cc' : g.t === 'MC' ? 'tag-mc' : g.t === 'FC1' ? 'tag-fc1' : g.t === 'FC2' ? 'tag-fc2' : 'tag-fc'}`}>{g.t}</span>
                            <span class={`b ${g.pri === 'high' ? 'br' : 'by'}`}>เป้าหมาย: Level {g.exp} (ปัจจุบัน {g.sup})</span>
                            <span class={`b ${g.pri === 'high' ? 'br' : 'by'}`} style={{ marginLeft: '4px' }}>{g.pri === 'high' ? 'เร่งด่วน' : 'ต้องพัฒนา'}</span>
                        </div>
          )}
                </div>
            </div>
...
export const EmployeeIDP = defineComponent({ name: "EmployeeIDP", props: Object as PropType<{learningMethods?: LearningMethodOption[];}>, setup(__props) {const { learningMethods = DEFAULT_LEARNING_METHOD_OPTIONS } = __props as any;
    ...
    <>
            ...
            <div key={m.key} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderLeft: `3px solid ${m.color}`, borderRadius: 'var(--r)', padding: '11px 14px' }}>
                                <div class="fw7 fs12" style={{ color: m.color, marginBottom: '4px' }}>{m.label}</div>
        ...
        return (
          <div key={g.cd} class="idp-gap" style={{ marginBottom: '20px', scrollMarginTop: '110px', overflow: 'hidden' }}>
                        <div class="idp-gap-h" style={{ padding: '14px 18px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, flexWrap: 'wrap' }}>
                                <span class={`b ${g.pri === 'high' ? 'br' : 'by'}`}>{g.pri === 'high' ? 'เร่งด่วน' : 'ต้องพัฒนา'}</span>
                                <span class={g.t === 'CC' ? 'tag-cc' : g.t === 'MC' ? 'tag-mc' : g.t === 'FC1' ? 'tag-fc1' : g.t === 'FC2' ? 'tag-fc2' : 'tag-fc'}>{g.t}</span>
                                <span class="fw8 fs14">{g.n}</span>
                                <span class="muted fs12" style={{ marginLeft: '4px' }}>คาดหวัง {g.exp} ปัจจุบัน {g.actual}</span>
                            </div>
            ...
            <div style={{ background: '#FEF2F2', borderBottom: '1px solid #FECACA', padding: '12px 18px', display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    <div class="fw7 fs13" style={{ color: 'var(--red)', marginBottom: '3px' }}>แผนนี้ไม่ผ่านการอนุมัติ</div>
                                    <div class="fs12" style={{ color: '#991B1B', marginBottom: '6px' }}>
                                        <span class="fw6">{g.rejectedBy}</span> {g.rejectedDate}
                                    </div>
                                    <div class="fs12" style={{ background: '#fff', border: '1px solid #FECACA', borderRadius: '6px', padding: '8px 10px', color: 'var(--text2)' }}>
                                        {g.rejectComment}
                                    </div>
                                </div>
                                <button class="btn btn-r btn-sm" style={{ flexShrink: 0 }}>แก้ไขแผน</button>
                            </div>
            }

                        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--blue-md)', background: '#fff' }}>
                            <label class="lbl">เป้าหมายการพัฒนา <span style={{ color: 'var(--red)' }}>*</span></label>
                            <textarea class="ta" style={{ minHeight: '52px', marginTop: '5px' }} placeholder="ระบุเป้าหมายการพัฒนา..." />
                        </div>
              ...
              <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text3)', fontSize: '13px', borderBottom: '1px solid var(--border)' }}>
                                    ...
                                    <div class="fw6 fs13" style={{ color: 'var(--text2)' }}>ยังไม่มีกิจกรรมพัฒนา</div>
                                </div>
              }
                        </div>

                        <div style={{ padding: '12px 18px', borderTop: '1px solid var(--border)', background: '#fff' }}>
                            <button class="btn btn-s btn-sm" type="button" onClick={() => toggleForm(idx)}>
            ...
            <div style={{ padding: '18px', background: '#F8FBFF', borderTop: '1px solid #DBE7F5', animation: 'slideDown .22s ease' }}>
                                ...
                                <div class="g2 mb12">
                                    <div class="fg" style={{ margin: 0 }}>
                                        <label class="lbl" style={{ fontSize: '11px' }}>เลือกกิจกรรมจาก Catalog (HR)</label>
                                        <select
                    class="sel"
                    ...
                    onChange={(e) => {
                      ...
                      setForm(idx, { catalog: val, title: val && val !== 'custom' ? val : '' });
                                    ...
                                    <div class="fg" style={{ margin: 0 }}>
                                        <label class="lbl" style={{ fontSize: '11px' }}>ประเภทการเรียนรู้ <span style={{ color: 'var(--red)' }}>*</span></label>
                                        <select
                    class="sel"
                    style={{ fontSize: '12px', marginTop: '4px' }}
                    value={form.method}
                    ...
                    <option key={method.key} value={method.key}>{method.label}</option>
                                ...
                                <div class="fg mb12">
                                    <label class="lbl" style={{ fontSize: '11px' }}>ชื่อกิจกรรม <span style={{ color: 'var(--red)' }}>*</span></label>
                                    <input
                  class="inp"
                  style={{ fontSize: '12px', marginTop: '4px' }}
                  value={form.title}
                  ...
                  placeholder="เช่น อบรม AI & Data Analytics หรือระบุกิจกรรมของตัวเอง" />
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                                    <div class="fg" style={{ margin: 0 }}>
                                        <label class="lbl" style={{ fontSize: '11px' }}>วันที่เริ่ม <span style={{ color: 'var(--red)' }}>*</span></label>
                                        <input type="date" class="inp" style={{ fontSize: '12px', marginTop: '4px' }} value={form.startDate} onChange={(e) => setForm(idx, { startDate: e.target.value })} />
                                    </div>
                                    <div class="fg" style={{ margin: 0 }}>
                                        <label class="lbl" style={{ fontSize: '11px' }}>วันที่สิ้นสุด <span style={{ color: 'var(--red)' }}>*</span></label>
                                        <input
                    type="date"
                    class="inp"
                  ...
                  <div style={{ color: 'var(--red)', fontSize: '11px', marginTop: '3px' }}>วันสิ้นสุดต้องอยู่หลังวันที่เริ่ม</div>
                                    ...
                                    <div class="fg" style={{ margin: 0 }}>
                                        <label class="lbl" style={{ fontSize: '11px' }}>น้ำหนัก (%) <span style={{ color: 'var(--red)' }}>*</span></label>
                                        <input
                    type="number"
                    class="inp"
                    min={1}
                  ...
                  <div style={{ color: 'var(--red)', fontSize: '11px', marginTop: '3px' }}>น้ำหนักต้องอยู่ระหว่าง 1-100</div>
                                ...
                                <div class="fg mb14">
                                    <label class="lbl" style={{ fontSize: '11px' }}>หมายเหตุ <span class="lbl-opt">(ไม่บังคับ)</span></label>
                                    <textarea class="ta" style={{ fontSize: '12px', minHeight: '52px', marginTop: '4px' }} value={form.note} onChange={(e) => setForm(idx, { note: e.target.value })} placeholder="อธิบายว่าทำไมถึงเลือกกิจกรรมนี้ หรือรายละเอียดเพิ่มเติม..." />
                                </div>

                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', paddingTop: '10px', borderTop: '1px solid var(--border)' }}>
                                    <button class="btn btn-s btn-sm" type="button" onClick={() => toggleForm(idx)}>ยกเลิก</button>
                                    <button class="btn btn-s btn-sm" type="button">บันทึกร่าง</button>
                                    <button
            ...
            <div class="flex g8 mt4" style={{ paddingTop: '4px' }}>
                <button class="btn btn-t" onClick={submitAllIDP}>{"ส่ง IDP ทั้งหมด"}</button>
                <button class="btn btn-s" onClick={saveIDPDraft}>{"บันทึกร่าง"}</button>
            </div>
        </>;} });
        ...
        return (
          <div key={g.cd} style={{ marginBottom: '24px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px', padding: '10px 14px', background: hasFailed ? '#FEF2F2' : 'var(--blue-lt)', border: `1px solid ${hasFailed ? '#FECACA' : 'var(--blue-md)'}`, borderLeft: `4px solid ${hasFailed ? 'var(--red)' : 'var(--blue)'}`, borderRadius: 'var(--r-lg)' }}>
                            ...
                            <span class={g.t === 'CC' ? 'tag-cc' : g.t === 'MC' ? 'tag-mc' : g.t === 'FC1' ? 'tag-fc1' : g.t === 'FC2' ? 'tag-fc2' : 'tag-fc'}>{g.t}</span>
                            <span class="fw7 fs13" style={{ color: hasFailed ? 'var(--red)' : 'inherit' }}>{g.n}</span>
              ...
              return (
                <div key={aIdx} class="card mb10">
                  ...
                  <div style={{ background: '#FEF2F2', borderTop: '1px solid #FECACA', borderBottom: '1px solid #FECACA', padding: '12px 18px', display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                                            <div style={{ fontSize: '20px', flexShrink: 0 }}>!</div>
                                            <div style={{ flex: 1 }}>
                                                <div class="fw7 fs13" style={{ color: 'var(--red)', marginBottom: '2px' }}>กิจกรรมนี้ไม่ผ่าน</div>
                                                <div class="fs12" style={{ color: '#991B1B', marginBottom: '6px' }}>
                                                    <span class="fw6">{act.rejectedBy}</span> · {act.rejectedDate}
                                                </div>
                                                <div class="fs12" style={{ background: '#fff', border: '1px solid #FECACA', borderRadius: '6px', padding: '8px 10px', color: 'var(--text2)', marginBottom: '10px' }}>
                                                    "{act.rejectComment}"
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '12px', marginBottom: '12px' }}>
                                                ...
                                                <div class="fg mb8">
                                                    <label class="lbl" style={{ fontWeight: 500, fontSize: '11px' }}>บันทึก</label>
                                                    <textarea
                          class="ta"
                          ...
                          onChange={(e) => setForm(g.cd, aIdx, { note: e.target.value })} />
                                                ...
                                                <div class="g2 mb8">
                                                    <div>
                                                        <div class="lbl mb8" style={{ fontSize: '11px' }}>แนบหลักฐาน <span class="lbl-opt">(ถ้ามี)</span></div>
                                                        <label class="upload-area" style={{ width: '100%', padding: '12px', background: '#fff', cursor: 'pointer', display: 'block' }}>
                                                            <input
                              type="file"
                              style={{ display: 'none' }}
                              onChange={(e) => {
                              ...
                              onChange={(e) => setForm(g.cd, aIdx, { evidenceDesc: e.target.value })} />
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '6px' }}>
                                                    <button class="btn btn-s btn-sm" type="button" onClick={() => saveProgress(g.cd, aIdx, 'draft')}>บันทึกร่าง</button>
                                                    <button class="btn btn-t btn-sm" type="button" onClick={() => saveProgress(g.cd, aIdx, 'saved')}>บันทึก</button>
                                                </div>
                                            </div>
                    }
                                        ...
                                        {act.logs.map((L, lIdx) =>
                    <div key={lIdx} class="flex ic g8" style={{ padding: '8px 0', borderBottom: '1px solid var(--border)', background: L.type === 'reject' ? '#FEF2F2' : 'transparent' }}>
    ...
    <>
                  ...
                  onClick={() => setExpandedId(isOpen ? null : act.id)}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                                            ...
                                            <span class={getGapTagClass(act.gapT)} style={{ fontSize: "9px" }}>{act.gapT}</span>
                                        </div>
                                    ...
                                    <span class={`b ${meta.badge}`} style={{ flexShrink: 0 }}>{meta.label}</span>
            ...
            <div class="card">
              ...
              return (
                ...
                <div class="cb" style={{ paddingTop: 0 }}>
              ...
              return (
                <div key={gap.cd} style={{ padding: "14px 0", borderBottom: "1px solid var(--border)" }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                                            <span class={getGapTagClass(gap.t)}>{gap.t}</span>
                                            <span class="fw7 fs13">{gap.n}</span>
                                            <span class="muted fs12">{gap.cd} · Gap {gap.gapValue}</span>
                                            <span class={`b ${statusMeta.cls}`} style={{ marginLeft: "auto" }}>{statusMeta.label}</span>
                                        </div>
                                        <div style={{ marginTop: "10px", display: "flex", alignItems: "center", gap: "10px" }}>
                                            <div class="pw" style={{ flex: 1, height: "6px" }}>
                                                <div class="pb" style={{ width: `${pct}%`, background: pct === 100 ? "var(--green)" : "var(--blue)" }} />
                                            </div>
          ...
          <div style={{ padding: "16px 0" }}>
            ...
            <div key={`${gap.cd}-${idx}`} style={{ padding: "14px 0", borderBottom: "1px solid var(--border)" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", marginBottom: "8px" }}>
                                        ...
                                        <span class="b bg" style={{ marginLeft: "auto" }}>เสร็จสิ้น</span>
                                    </div>
                                    {gap.acts.map((act: any, actIdx: number) =>
              <div key={actIdx} style={{ marginBottom: "8px", padding: "8px 12px", background: "var(--bg)", borderRadius: "var(--r)", borderLeft: "3px solid var(--green)" }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
                                                ...
                                                <span class="b bg" style={{ marginLeft: "auto", fontSize: "9px" }}>ผ่านแล้ว ✓</span>
                                            </div>
                                            {act.logs.map((log: any, logIdx: number) =>
