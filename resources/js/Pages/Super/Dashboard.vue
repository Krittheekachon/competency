...
const selectedAssessment = computed(() => selectedStaff.value);
...
const selectAssessmentStaff = (person) => {
    selectedStaff.value = selectedStaff.value?.sso === person.sso ? null : person;
            ...
            <div class="content">
                ...
                <template v-else-if="activePage === 'dh-idp'">
                    <template v-if="!selectedIdpPerson">
                        ...
                        <div v-for="workGroup in visibleIdpGroups" :key="`idp-${workGroup.work}`" class="card org-list-card">
                            <div class="org-list-head">
                                <div class="fw8 fs14">{{ workGroup.work }}</div>
                                <span class="b bgr">{{ workGroup.count }} คน</span>
                            </div>
                            <div v-for="unitGroup in workGroup.units" :key="`idp-${workGroup.work}-${unitGroup.unit}`" class="org-unit-section">
                                <div class="org-unit-title">{{ unitGroup.unit }} <span class="muted fw6">({{ unitGroup.count }} คน)</span></div>
                                <div v-for="row in unitGroup.members" :key="row.sso" class="team-row org-member-row">
                        ...
                        <template v-if="!selectedGapPerson">
                            ...
                            <div class="g2 team-grid">
                                ...
                                <div class="card">
                                    ...
                                    <div v-for="workGroup in gapRowGroups" :key="`gap-${workGroup.work}`" class="org-unit-section">
                                        <div class="org-list-head compact">
                                            <div class="fw8 fs13">{{ workGroup.work }}</div>
                                            <span class="b bgr">{{ workGroup.count }} คน</span>
                                        </div>
                                        <div v-for="unitGroup in workGroup.units" :key="`gap-${workGroup.work}-${unitGroup.unit}`">
                                            <div class="org-unit-title">{{ unitGroup.unit }} <span class="muted fw6">({{ unitGroup.count }} คน)</span></div>
                                            <button
                                                v-for="person in unitGroup.members"
                                                :key="person.sso"
                ...
                <template v-else-if="activePage === 'dh-assess'">
                    ...
                    <div class="assessment-workspace">
                        <div class="card assess-list-card accordion-list-card">
                            <div class="ch">
                                <div>
                                    <div class="ct">งาน / หน่วยงาน / บุคลากร</div>
                                    <div class="cs">
                                        กดชื่องานเพื่อเปิดหน่วยงาน กดหน่วยงานเพื่อเปิดรายชื่อ และกดชื่อบุคลากรเพื่อดูรายละเอียดด้านล่าง
                                    </div>
                                </div>
                                <button v-if="selectedAssessmentWork" class="btn btn-s btn-sm" type="button" @click="resetAssessmentDrill">ปิดทั้งหมด</button>
                            </div>

                            <div class="accordion-list">
                                <div v-for="workGroup in teamMemberGroups" :key="`assess-work-${workGroup.work}`" class="accordion-work">
                                    <button
                                        class="accordion-trigger work-trigger"
                                        :class="{ open: selectedAssessmentWork === workGroup.work }"
                                        type="button"
                                        @click="selectAssessmentWork(workGroup.work)"
                                    >
                                        <div>
                                            <div class="fw8 fs14">{{ workGroup.work }}</div>
                                            <div class="muted fs11">{{ workGroup.units.length }} หน่วยงาน</div>
                                        </div>
                                        <span class="b bb">{{ workGroup.count }} คน</span>
                                    </button>

                                    <div v-if="selectedAssessmentWork === workGroup.work" class="accordion-panel">
                                        <div v-for="unitGroup in workGroup.units" :key="`assess-unit-${workGroup.work}-${unitGroup.unit}`" class="accordion-unit">
                                            <button
                                                class="accordion-trigger unit-trigger"
                                                :class="{ open: selectedAssessmentUnit === unitGroup.unit }"
                                                type="button"
                                                @click="selectAssessmentUnit(unitGroup.unit)"
                                            >
                                                <div>
                                                    <div class="fw8 fs13">{{ unitGroup.unit }}</div>
                                                    <div class="muted fs11">{{ workGroup.work }}</div>
                                                </div>
                                                <span class="b bgr">{{ unitGroup.count }} คน</span>
                                            </button>

                                            <div v-if="selectedAssessmentUnit === unitGroup.unit" class="accordion-panel people-panel">
                                                <template v-for="person in unitGroup.members" :key="person.sso">
                                                    <button
                                                        class="assess-person"
                                                    ...
                                                    <div v-if="selectedAssessment?.sso === person.sso" class="assessment-detail card inline-assessment-detail">
                                                        <div class="flex ic jb mb12">
                                                            ...
                                                            <span class="b" :class="activeDraft.submitted ? 'bt' : 'bgr'">
                                                                {{ activeDraft.submitted ? 'ยืนยันผลการประเมินแล้ว' : (assessmentSavedAt || 'ยังไม่มีการแก้ไขผลการประเมิน') }}
                                                            </span>
                                                        </div>
                                                        ...
                                                        <div v-for="(comp, index) in filteredComps" :key="comp.id" class="card assessment-card">
                                                            ...
                                                            <div class="assessment-columns assessment-columns-three">
                                                                <div class="assessment-side self">
                                                                    <div class="bc fw8 fs12 mb10">1. บุคลากรประเมินตนเอง</div>
                                                                ...
                                                                <div class="assessment-side supervisor">
                                                                    <div class="tc fw8 fs12 mb10">2. หัวหน้างานประเมิน</div>
                                                                    <div class="muted fw7 fs11 mb6">พฤติกรรมบ่งชี้ (ใช้ประกอบการตัดสิน)</div>
                                                                    <ul class="behavior-list">
                                                                        <li v-for="behavior in comp.behaviors[getSupervisorScore(comp)]" :key="behavior">{{ behavior }}</li>
                                                                    </ul>
                                                                    <div class="hint-box">แสดงพฤติกรรมบ่งชี้ตามคะแนนที่หัวหน้างานเลือก ระดับ {{ getSupervisorScore(comp) }}: {{ scoreLabels[getSupervisorScore(comp) - 1] }}</div>

                                                                    <div class="muted fw7 fs11 mb8">คะแนนความสามารถของบุคลากรโดยหัวหน้างาน</div>
                                                                    <div class="score-grid">
                                                                        <div v-for="score in [1, 2, 3, 4, 5]" :key="score" class="score-tile" :class="{ selected: getSupervisorScore(comp) === score }">
                                                                            <span>{{ score }}</span>
                                                                            <small>{{ scoreLabels[score - 1] }}</small>
                                                                        </div>
                                                                    </div>

                                                                    <div class="muted fw7 fs11 mb8">ข้อเสนอแนะจากหัวหน้างาน</div>
                                                                    <div class="hint-box">{{ getSupervisorFeedback(comp) }}</div>
                                                                </div>

                                                                <div class="assessment-side reviewer">
                                                                    <div class="bc fw8 fs12 mb10">3. ผู้บังคับบัญชา (คุณ) *</div>
                                                                    <div class="muted fw7 fs11 mb6">พฤติกรรมบ่งชี้ (ใช้ประกอบการตัดสิน)</div>
                                                                    <ul class="behavior-list">
                                                                        <li v-for="behavior in comp.behaviors[getReviewerScore(comp)]" :key="behavior">{{ behavior }}</li>
                                                                    </ul>
                                                                    <div class="hint-box">แสดงพฤติกรรมบ่งชี้ตามคะแนนที่คุณเลือก ระดับ {{ getReviewerScore(comp) }}: {{ scoreLabels[getReviewerScore(comp) - 1] }}</div>

                                                                    <div class="muted fw7 fs11 mb8">คะแนนความสามารถของบุคลากรโดยผู้บังคับบัญชา</div>
                                                                    <div class="score-grid">
                                                                        <button
                                                                    ...
                                                                    <div v-if="index === filteredComps.length - 1" class="approval-flow">
                                                                        <div class="flow-step"><div class="av s24 flow-dot">✓</div><div><div class="fw7 fs12">ยืนยันผลการประเมิน</div><div class="muted fs11">ระบบจะใช้ผลนี้สำหรับขั้นตอนถัดไป</div></div></div>
                                                        ...
                                                        <div class="flex g8 mt16">
                                                            ...
                                                            <button class="btn btn-t" type="button" style="flex: 2" :disabled="activeDraft.submitted" @click="submitAssessmentToManager">
                                                                {{ activeDraft.submitted ? 'ยืนยันผลการประเมินแล้ว' : 'ยืนยันผลการประเมิน' }}
                                                            </button>
                                                        </div>
                                                    </div>
                                                </template>
                                ...
                                <div v-if="teamMemberGroups.length === 0" class="empty-card">
                                    ยังไม่มีข้อมูลบุคลากรในความดูแล
                                </div>
                            </div>
                        </div>
                    </div>
