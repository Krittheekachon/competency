    ...
    <>
      ...
      <>
                    {!hideSupervisorHeader && <div class="card mb16">
                        <div class="cb" style={{ padding: "12px 16px" }}>
                            ...
                            <div class="flex ic" style={{ justifyContent: "flex-end", marginTop: "10px" }}>
                                <div class="flex ic g8">
                                    <div class="av" style={{ width: "34px", height: "34px", background: "var(--orange)", fontSize: "12px" }}>{currentUser.n[0]}</div>
                                    <div class="flex ic g4" style={{ flexWrap: "wrap" }}>
                                        <div class="fw8 fs12">{currentUser.t}{currentUser.n}</div>
                                        <div class="muted fs11">· {currentUser.p} · {currentUser.d}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="g4 mb20" style={{ gridTemplateColumns: "repeat(5,minmax(0,1fr))" }}>
                        ...
                        <div class="sc" style={{ borderTop: "3px solid var(--yellow)" }}><div class="sl">รอคุณประเมิน</div><div class="sv yc">{assessCounts.pending}</div><div class="ss muted">คน</div></div>
                        <div class="sc" style={{ borderTop: "3px solid var(--teal)" }}><div class="sl">หัวหน้าฝ่าย/คณบดี</div><div class="sv tc">{assessCounts.forwarded}</div><div class="ss muted">อยู่ระหว่างพิจารณา</div></div>
                        <div class="sc" style={{ borderTop: "3px solid var(--green)" }}><div class="sl">เสร็จสมบูรณ์</div><div class="sv gcc">{assessCounts.done}</div><div class="ss muted">คน</div></div>
                    </div>
      ...
      <div class="flex ic jb mb20">
                    ...
                    <div class="flex ic g8">
                        ...
                        <select class="ta" style={{ width: 'fit-content', padding: '6px 10px', fontSize: '13px' }} value={activeDept} onChange={(e) => setActiveDept(e.target.value)}>
            ...
            <div class="g2" style={supervisorMode ? { gridTemplateColumns: "340px minmax(0,1fr)", alignItems: "start" } : undefined}>
                <div class="card">
                ...
                <div key={u.sso || i} class={`flex ic g12 pointer transition-all ${isSel ? 'bg-blue-lt' : 'hover-bg bg-white'}`} style={{ borderBottom: '1px solid var(--border)', minHeight: "72px", padding: "12px 14px" }} onClick={() => openStaff(u)}>
                                    ...
                                    <div class="flex-1" style={{ minWidth: 0 }}>
                                        <div class="flex jb g8" style={{ alignItems: "center" }}>
                                            <div class="fw7 fs13" style={{ color: "var(--navy)", lineHeight: 1.35, minWidth: 0 }}>{u.t}{u.n}</div>
                                            {statusBadge(u)}
                                        </div>
                                        <div class="muted fs11" style={{ lineHeight: 1.3, marginTop: "2px" }}>{u.p}</div>
                                    </div>
                                </div>);
                  ...
                  return (
                    <div key={u.sso || i} class={`flex ic g12 p12 pointer transition-all ${isSel ? 'bg-blue-lt' : 'hover-bg bg-white'}`} style={{ borderBottom: '1px solid var(--border)', borderLeft: isSel ? '4px solid var(--blue)' : '4px solid transparent' }} onClick={() => openStaff(u)}>
                                            ...
                                            <div class="flex-1 overflow-hidden">
                                                ...
                                                <div class="flex ic g4 truncate"><span class="fs10 muted fw5">{u.p}</span></div>
                                            </div>
                                        </div>);
        ...
        <div class={supervisorMode ? "" : "card"} style={supervisorMode ? { minWidth: 0 } : undefined}>
            ...
            <Fragment key={c.id}>
              ...
              <div class="card mb20" style={{ overflow: "hidden", boxShadow: "var(--sh)" }}>
                                        <div class="flex ic jb" style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", background: "#fff" }}>
                                            <div class="flex ic g10">
                                                <span class={c.tg}>{c.t}</span>
                                                <span class="fw8 fs14">{c.n}</span>
                                            </div>
                                            <div class="fw8 tc fs12">{c.ss}/5 ✓</div>
                                        </div>
                      ...
                      <div key={value} style={{ height: "54px", border: `1.5px solid ${c.ss === value ? "var(--teal)" : "var(--border)"}`, borderRadius: "8px", background: c.ss === value ? "var(--teal-lt)" : "#fff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                                                            <span class="fw8 fs15" style={{ color: c.ss === value ? "var(--teal)" : "var(--text3)" }}>{value}</span>
                                                            <span class="fs10 muted">{scoreLabels[value - 1]}</span>
                                                        </div>
                      )}
                                                </div>
                                                ...
                                                <div style={{ display: "grid", gridTemplateColumns: "minmax(180px,1fr) 1fr", gap: "14px", alignItems: "center" }}>
                                                    ...
                                                    <div class="fs11">
                                                        <div class="mb8"><span class="fw8">URL:</span> <span class="bc">{c.evidence?.url}</span></div>
                                                        <div><span class="fw8">คำอธิบาย:</span> <span class="muted">{c.evidence?.desc}</span></div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        return (
                          <button key={value} type="button" onClick={() => handleMark(c.id, value)} style={{ minHeight: "54px", border: `1.5px solid ${selected ? "var(--teal)" : "var(--border)"}`, borderRadius: "9px", background: selected ? "var(--teal-lt)" : "#fff", color: selected ? "var(--teal)" : "var(--text3)", fontFamily: "inherit", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                                                                <span class="fw8 fs15">{value}</span>
                                                                <span class="fs10">{scoreLabels[value - 1]}</span>
                                                            </button>);

                      })}
                                                </div>
                                                <textarea class="ta" style={{ minHeight: "80px", fontSize: "13px", borderRadius: "10px" }} placeholder="ใส่คำเสนอแนะ (ถ้ามี)..." value={feedback[c.id] || ""} onChange={(e) => handleFeedback(c.id, e.target.value)} />
                                                <div style={{ borderTop: "1px solid var(--border)", marginTop: "24px", paddingTop: "16px" }}>
                                                    <div class="flex ic g10 mb12"><div class="av s24" style={{ background: "var(--blue-lt)", color: "var(--blue)", fontSize: "10px" }}>2</div><div><div class="fw7 fs12">หัวหน้าฝ่าย</div><div class="muted fs11">รอดำเนินการ...</div></div></div>
                                                    <div class="flex ic g10"><div class="av s24" style={{ background: "var(--blue-lt)", color: "var(--blue)", fontSize: "10px" }}>3</div><div><div class="fw7 fs12">คณบดี</div><div class="muted fs11">รอยืนยัน</div></div></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div> :
              ...
              <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? '#fff' : 'rgba(0,0,0,0.02)' }}>
                                        <div class="flex ic jb mb10">
                                            <div class="flex ic g8">
                                                <span class={c.tg} style={{ borderRadius: '4px', padding: '2px 8px' }}>{c.t}</span>
                                                <span class="fw7 fs14" style={{ color: 'var(--navy)' }}>{c.n}</span>
                                            </div>
                                            <span class="fw8 fs16 bc">{c.ss}/5</span>
                                        </div>
                                    </div>
              }
                            </Fragment>
...
export const TeamGap = defineComponent({ name: "TeamGap", props: ["users", "currentUser", "supervisorUsers", "onSupervisorChange", "hideSupervisorHeader"], setup(__props) {const { users = [], currentUser = {}, supervisorUsers, onSupervisorChange, hideSupervisorHeader = false } = __props as any;
    ...
    if (supervisorMode && selectedGapPerson) return (
      <>
            <button class="btn btn-s btn-sm mb16" onClick={() => setSelectedGapId(null)}>← กลับ</button>

            <div class="card mb16">
                <div class="cb flex ic jb g12" style={{ padding: "18px 20px", flexWrap: "wrap" }}>
                    <div class="flex ic g12">
                        <div class="av" style={{ width: "48px", height: "48px", background: "var(--navy)", fontSize: "16px" }}>{selectedGapPerson.n[0]}</div>
                        <div>
                            <div class="fw8 fs15">{selectedGapPerson.n}</div>
                            <div class="muted fs12">{selectedGapPerson.p}</div>
                            <div class="mt6"><span class="b br">ไม่ผ่าน {gapDetailRows.length} สมรรถนะ</span></div>
                        </div>
                    </div>
                    <div class="flex g6">
                        <button class="btn btn-s btn-sm" disabled={selectedGapIndex <= 0} onClick={() => setSelectedGapId(gapPeople[selectedGapIndex - 1]?.id)}>← ก่อนหน้า</button>
                        <button class="btn btn-s btn-sm" disabled={selectedGapIndex >= gapPeople.length - 1} onClick={() => setSelectedGapId(gapPeople[selectedGapIndex + 1]?.id)}>ถัดไป →</button>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb16" style={{ overflow: "hidden" }}>
              ...
              <tr key={row.n}>
                                <td class="fw7 fs13">{row.n}</td>
                                <td style={{ textAlign: "center" }}><span class={row.tg}>{row.t}</span></td>
                                <td style={{ textAlign: "center" }}><span class="av s32" style={{ display: "inline-flex", background: "var(--navy)", fontSize: "13px" }}>{row.expected}</span></td>
                                <td style={{ textAlign: "center" }}><span class="av s32" style={{ display: "inline-flex", background: "var(--blue-lt)", color: "var(--blue)", fontSize: "13px" }}>{row.self}</span></td>
                                <td style={{ textAlign: "center" }}><span class="av s32" style={{ display: "inline-flex", background: "var(--red-bg)", color: "var(--red)", fontSize: "13px" }}>{row.supervisor}</span></td>
                                <td style={{ textAlign: "center" }}><span class="rc fw8">{row.diff}</span></td>
                                <td style={{ textAlign: "center" }}><span class="b br">ไม่ผ่าน</span></td>
                            </tr>
              )}
                    </tbody>
                </table>
            ...
            <div class="card" style={{ overflow: "hidden" }}>
                ...
                <div style={{ padding: "16px 18px" }}>
                    <div class="tc fw7 fs12 mb10">● หัวหน้าฝ่าย</div>
                    <div style={{ background: "var(--teal-lt)", borderRadius: "9px", padding: "12px 14px" }} class="fs12">ยังไม่มีข้อเสนอแนะ</div>
                </div>
            </div>
        </>);
    ...
    if (supervisorMode) return (
      <>
            ...
            <div class="g3 mb16">
                <div class="sc" style={{ borderTop: "3px solid var(--navy)" }}><div class="sl">บุคลากรทั้งหมด</div><div class="sv">{gapPeople.length}</div><div class="ss muted">คน</div></div>
                <div class="sc" style={{ borderTop: "3px solid var(--blue)" }}><div class="sl">ประเมินแล้ว</div><div class="sv bc">{assessedPeople.length}</div><div class="ss muted">คน</div></div>
                <div class="sc" style={{ borderTop: "3px solid var(--red)" }}><div class="sl">พบ Gap</div><div class="sv rc">{foundGapPeople.length}</div><div class="ss muted">คน</div></div>
            </div>

            <div class="g2" style={{ gridTemplateColumns: "1fr 1fr", alignItems: "stretch" }}>
                <div class="card" style={{ overflow: "hidden" }}>
              ...
              <div key={comp.n} style={{ padding: "16px 18px", borderBottom: "1px solid var(--border)" }}>
                                <div class="flex ic g10">
                                    <span class={comp.tg}>{comp.t}</span>
                                    <div style={{ flex: 1 }}>
                                        <div class="flex ic jb g8">
                                            <div class="fw8 fs13">{comp.n}</div>
                                            <div class="rc fw8 fs12">{comp.count} คน</div>
                                        </div>
                                        <div class="pw mt8" style={{ height: "6px" }}>
                                            <div class="pb" style={{ width: `${Math.max(14, comp.count / gapPeople.length * 100)}%`, background: "var(--red)" }} />
                                        </div>
                                    </div>
                                </div>
                            </div>
              ...
              <div key={person.id} class={`flex ic g12 ${person.pending ? "" : "pointer hover-bg"}`} style={{ padding: "15px 18px", borderBottom: "1px solid var(--border)" }} onClick={() => !person.pending && setSelectedGapId(person.id)}>
                                <div class="av" style={{ width: "38px", height: "38px", background: person.pending ? "var(--text3)" : "var(--navy)", fontSize: "13px", flexShrink: 0 }}>{person.n[0]}</div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div class={`fw8 fs13 ${person.pending ? "muted" : ""}`}>{person.n}</div>
                                    {person.pending ?
    ...
    <>
            ...
            <div class="card mb14">
                <div class="ch"><div class="ct">ระดับสมรรถนะรายบุคคลแยกตามหน่วย</div></div>
                <div style={{ padding: 0 }}>
                    {DEPT_STRUCTURE[activeDept as keyof typeof DEPT_STRUCTURE].map((w, wi) =>
          <div key={wi}>
            ...
            <div key={ui}>
                  ...
                  <tr key={k}>
                                                    <td>
                                                        <div class="flex ic g8">
                                                            <div class="av s24" style={{ background: 'var(--navy)', color: '#fff', fontSize: '10px' }}>{u.n[0]}</div>
                                                            <div class="fw6 fs12" style={{ color: 'var(--navy)' }}>{u.n}</div>
                                                        </div>
                                                    </td>
                                                    {u.v.map((val, vi) => <td key={vi} style={{ textAlign: 'center', fontWeight: 700, color: 'var(--blue)' }}>{val}</td>)}
                                                    <td><span class={`b ${u.c}`} style={{ fontSize: '10px' }}>{u.pri}</span></td>
                                                </tr>
                  )}
                                        </tbody>
                                    </table>
            ...
            <div class="card">
                <div class="ch"><div class="ct">สมรรถนะที่ทีมต้องพัฒนามากที่สุด</div></div>
                <div class="cb">
                    {topDevs.map((d, i) =>
          <div key={i} class="flex ic g12 mb12">
                            <div style={{ flex: 1 }}><div class="fw6 fs13">{d.n}</div></div>
                            <div style={{ width: '180px' }}>
                                <div class="pw" style={{ height: '7px' }}>
                                    <div class="pb" style={{ width: `${d.rate}%`, background: d.rate < 60 ? 'var(--red)' : d.rate < 80 ? 'var(--yellow)' : 'var(--teal)' }}></div>
                                </div>
                            </div>
                            <span class={`b ${d.c}`} style={{ width: '80px', justifyContent: 'center' }}>{d.rate}% ประเมินแล้ว</span>
                        </div>
          )}
                </div>
            </div>
...
const LegacyTeamIDP = defineComponent({ name: "LegacyTeamIDP", props: Object as PropType<{users: any[];}>, setup(__props) {const { users } = __props as any;
    ...
    <>
            ...
            <div class="tab-bar">
                {tabsSource.map((t) =>
        <div key={t.id} class={`tab ${currentTab.value === t.id ? 'on' : ''}`} onClick={() => setCurrentTab(t.id)}>
                        {t.lb} <span class={`b ${t.c}`} style={{ marginLeft: '4px' }}>{t.count}</span>
                    </div>
        )}
            </div>
              ...
              <div key={s.id} class="card mb12 hover-bg transition-all pointer">
                                                <div class="cb">
                                                    <div class="flex ic g12 mb12">
                                                        <div class="av s40" style={{ background: 'var(--navy)', color: '#fff', fontSize: '14px', flexShrink: 0 }}>{s.av}</div>
                                                        <div style={{ flex: 1 }}>
                                                            <div class="fw8 fs14" style={{ color: 'var(--navy)' }}>{s.n}</div>
                                                            <div class="muted fs12">{s.p}</div>
                                                            <div class="muted fs10 mt2">ส่ง {s.d} · {s.acts} กิจกรรม</div>
                                                    ...
                                                    <div class="fg mb0">
                                                        <textarea class="ta" style={{ minHeight: '38px', fontSize: '12px' }} placeholder="Feedback..." value={feedbacks.value[s.id] || ""} onChange={(e) => setFeedbacks({ ...feedbacks.value, [s.id]: e.target.value })} />
                                                    </div>
                                                </div>
                                            </div>
              )}
      ...
      <div class="card">
                    <div class="cb text-center py-20 text-text3">
...
const SupervisorIDPHeader = defineComponent({ name: "SupervisorIDPHeader", props: ["currentUser", "supervisorUsers", "onSupervisorChange"], setup(__props) {const { currentUser, supervisorUsers, onSupervisorChange } = __props as any;
    ...
    <div class="card mb16">
            <div class="cb flex ic jb g12" style={{ padding: "12px 16px", flexWrap: "nowrap" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div class="fw8 fs13">{supervisorUnit}</div>
                ...
                <div class="flex ic" style={{ justifyContent: "flex-end", flex: "0 0 auto", minWidth: "fit-content" }}>
                    <div class="flex ic g8">
              ...
              return (
                <div key={gap.cd} class="card mb14" style={{ overflow: "hidden" }}>
                                    <div class="ch" style={{ justifyContent: "space-between", flexWrap: "wrap" }}>
                                        <div class="flex ic g8" style={{ flexWrap: "wrap" }}>
                                            <span class={gap.tg}>{gap.t}</span>
                                            <div class="ct">{gap.n}</div>
                                            <span class="muted fs12">คาดหวัง {gap.exp} · ได้ {gap.got} · <span class="rc fw7">Gap {gap.got - gap.exp}</span></span>
                                        </div>
                                        {decision && <span class={`b ${decision === "approved" ? "bg" : "br"}`}>{decision === "approved" ? "แผนผ่าน" : "แผนไม่ผ่าน"}</span>}
                                    </div>
                                    <div class="cb">
                                        <div class="fs12 fw7 mb4">เป้าหมายการพัฒนา</div>
                                        <div class="muted fs13 mb12" style={{ padding: "10px 12px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--r)" }}>{gap.goal}</div>
                                        <div class="flex g6 mb12" style={{ flexWrap: "wrap" }}>
    ...
    <>
        ...
        <button
          ...
          onClick={() => setActiveTab(tab.id)}>
                        ...
                        <div class="sl" style={{ fontSize: "11px", marginBottom: "8px" }}>{tab.label}</div>
                        <div class={`sv ${tab.id === "notsent" ? "" : tab.id === "pending" ? "yc" : tab.id === "rejected" ? "rc" : tab.id === "inprogress" ? "bc" : "gcc"}`} style={{ fontSize: "22px" }}>{getTabCount(tab.id)}</div>
                        <div class="ss muted" style={{ marginTop: "4px" }}>{"คน"}</div>
                    </button>
        )}
            </div>
            ...
            <div class="tab-bar">
                {tabs.map((tab) =>
        <div key={tab.id} class={`tab ${activeTab === tab.id ? "on" : ""}`} onClick={() => setActiveTab(tab.id)}>
                        {tab.label} <span class={`b ${tab.c}`} style={{ marginLeft: "4px" }}>{getTabCount(tab.id)}</span>
                    </div>
        )}
            </div>
    ...
    return (
      <div key={gap.cd} class="flex ic g12" style={{ padding: "12px 0", borderBottom: "1px dashed var(--border)", flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: 1, minWidth: "180px" }}>
                        <div class="fw7 fs13">{gap.n}</div>
                        <div class="muted fs11">ระดับคาดหวัง {gap.exp} · ระดับที่ได้ {gap.got}</div>
                        {decision === "rejected" && feedback?.[key] && <div class="fs11 rc mt4">เหตุผล: {feedback[key]}</div>}
                    </div>
                    <span class="fw8 rc fs13">Gap {gap.got - gap.exp}</span>
                </div>);
...
<div style={{ borderLeft: "4px solid var(--green)", borderRadius: "var(--r)", background: "#fbfdff", padding: "12px 14px", marginBottom: "12px" }}>
        <div class="flex ic jb mb8" style={{ flexWrap: "wrap", gap: "8px" }}>
            <div class="flex ic g8">
                <span class="fw8 fs13">{activity.title}</span>
                <span class="muted fs11">{activity.method}</span>
            </div>
            <span class="b bg">{activity.status}</span>
        </div>
  ...
  <div key={`${activity.title}-${event.date}-${event.text}`} style={{ display: "grid", gridTemplateColumns: "58px minmax(220px, 1fr) 150px", gap: "10px", borderTop: "1px solid var(--border)", padding: "9px 0" }}>
                <span class="muted fs11">{event.date}</span>
                <div>
                    <div class={`fs12 ${event.text.includes("ผ่าน") ? "gcc fw7" : ""}`}>{event.text}</div>
                    <span class="b bgr mt4" style={{ whiteSpace: "normal" }}>{event.file}</span>
                </div>
                <span class="muted fs11" style={{ textAlign: "right" }}>by {event.by}</span>
            </div>
  )}
    </div>;
...
const DeptHeadIDP = defineComponent({ name: "DeptHeadIDP", props: Object as PropType<any>, setup(__props) {
    ...
    if (selected) {
      ...
      return (
        <>
                <div class="flex ic jb mb16" style={{ flexWrap: "wrap", gap: "10px" }}>
                    <button class="btn btn-s btn-sm" onClick={() => setSelectedId(null)}>กลับรายการหัวหน้าฝ่าย</button>
                    <span class={`b ${meta.badge}`}>{meta.label}</span>
                </div>
                <div class="card mb14">
                    <div class="cb flex ic g12" style={{ flexWrap: "wrap" }}>
                        <div class="av" style={{ width: "44px", height: "44px", background: "var(--teal)", fontSize: "16px" }}>{selected.n[0]}</div>
                        <div style={{ flex: 1 }}>
                            <div class="fw8 fs15">{selected.n}</div>
                            <div class="muted fs12">{selected.p} · {selected.home}</div>
          ...
          <div key={gap.cd} class="card mb14" style={{ overflow: "hidden" }}>
                        ...
                        <div class="cb">
                            <div class="fs12 fw7 mb4">เป้าหมายการพัฒนา</div>
                            <div class="muted fs13 mb12" style={{ padding: "10px 12px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--r)" }}>{gap.goal}</div>
                            <div class="flex g6 mb12" style={{ flexWrap: "wrap" }}>{gap.acts.map((act) => <span key={act} class="b bb">{act}</span>)}</div>
                            {(canPlanReview || canActivityReview) &&
              <>
                                    <textarea class="ta" style={{ minHeight: "46px" }} value={feedback.value[selected.id] || ""} placeholder="ข้อเสนอแนะ / เหตุผลเมื่อส่งกลับ" onChange={(event) => setFeedback((prev) => ({ ...prev, [selected.id]: event.target.value }))} />
                                    <div class="flex g6 mt8">
                                        <button class="btn btn-p btn-sm" onClick={() => canPlanReview ? approve(selected.id) : setPhases((prev) => ({ ...prev, [selected.id]: "done" }))}>{canPlanReview ? "ผ่านแผนและส่งต่อคณบดี" : "ผ่านกิจกรรม"}</button>
                                        <button class="btn btn-r btn-sm" onClick={() => returnPlan(selected.id)}>ส่งกลับ</button>
                                    </div>
                                </>
              }
                        </div>
    ...
    <>
            ...
            <div class="card mb14">
                <div class="cb flex ic jb" style={{ flexWrap: "wrap", gap: "10px" }}>
                    <div class="flex ic g6" style={{ flexWrap: "wrap" }}>
                        <button class={`btn ${scope.value === "unit" ? "btn-p" : "btn-s"} btn-sm`} onClick={() => {setScope("unit");setActiveHome("");setSelectedId(null);}}>หน่วยงาน</button>
                        <button class={`btn ${scope.value === "branch" ? "btn-p" : "btn-s"} btn-sm`} onClick={() => {setScope("branch");setActiveHome("");setSelectedId(null);}}>สาขาวิชา</button>
                        <select class="ta" style={{ width: "fit-content", minWidth: "220px", padding: "7px 10px", fontSize: "12px" }} value={selectedHome} onChange={(event) => {setActiveHome(event.target.value);setSelectedId(null);}}>
                            {homes.map((home) => <option key={home} value={home}>{home}</option>)}
                        </select>
                    </div>
                    <div class="muted fs12">{scope.value === "unit" ? "รายการจากหน่วยงานภายใต้ฝ่าย" : "รายการจากสาขาวิชา"} · {selectedHome}</div>
                </div>
            </div>
            <div class="g3 mb14">
                {tabs.map((tab) =>
        <button key={tab.id} class="sc" type="button" style={{ textAlign: "left", cursor: "pointer", borderColor: activeTab.value === tab.id ? "var(--blue)" : "var(--border)", fontFamily: "inherit" }} onClick={() => setActiveTab(tab.id)}>
                        <div class="sl">{tab.label}</div>
                        <div class="sv"><span class={`b ${tab.c}`}>{count(tab.id)}</span></div>
                    </button>
        )}
            </div>
            <div class="tab-bar">
        ...
        return (
          <div key={person.id} class="card mb12">
                        <div class="cb flex ic g12" style={{ flexWrap: "wrap" }}>
                            <div class="av" style={{ width: "40px", height: "40px", background: "var(--teal)", fontSize: "14px" }}>{person.n[0]}</div>
                            <div style={{ flex: 1, minWidth: "220px" }}>
                                <div class="fw8 fs14">{person.n}</div>
                                <div class="muted fs12">{person.p} · {person.home}</div>
                            </div>
                            <span class={`b ${meta.badge}`}>{meta.label}</span>
                            <button class={`btn ${actionable ? "btn-p" : "btn-s"} btn-sm`} onClick={() => setSelectedId(person.id)}>{actionable ? "ตรวจสอบ" : "ดูข้อมูล"}</button>
                        </div>
                    </div>);
